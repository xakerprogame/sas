...
SkyWrathHake.IsToggled = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake"}, "Enabled", "")
SkyWrathHake.IsTargetParticleEnabled = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake"}, "Target indicator", "Draws particle for current target")
SkyWrathHake.IsConcShotParticleEnabled = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake"}, "Concussive shot indicator", "Draws concussive shot particle if enemy is in range of cast")
...
SkyWrathHake.enemyInRange = Menu.AddOptionSlider({"Hero Specific", "SkyWrathHake"}, "Closest to mouse range", 100, 600, 100)
...
for k, v in pairs(SkyWrathHake.menuItems) do
	SkyWrathHake.ItemsOptionID[k] = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake", "Magic Damage Items"}, SkyWrathHake.menuItems[k], "")
end
...
for k, v in pairs(SkyWrathHake.menuAbilities) do
	SkyWrathHake.AbilitiesOptionID[k] = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake", "Abilities"}, SkyWrathHake.menuAbilities[k], "")
end

SkyWrathHake.IsPopLinkenToggled = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake", "Pop Linkens Items"}, "Enabled Pop Linken", "")
...
for k, v in pairs(SkyWrathHake.menuPopupLinkens) do
	SkyWrathHake.PopLinkensOptionID[k] = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake", "Pop Linkens Items"}, SkyWrathHake.menuPopupLinkens[k], "")
end

SkyWrathHake.IsPopAMReflectToggled = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake", "Pop Antimage's Reflect Items"}, "Enabled Pop AM Reflect", "")
...
for k, v in pairs(SkyWrathHake.menuPopupAMReflect) do
	SkyWrathHake.PopupAMReflectOptionID[k] = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake", "Pop Antimage's Reflect Items"}, SkyWrathHake.menuPopupAMReflect[k], "")
end

SkyWrathHake.IsBMToggled = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake"}, "Check for BladeMail", "")
SkyWrathHake.IsSRToggled = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake"}, "Use Soul Ring", "")
SkyWrathHake.IsBlinkToggled = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake"}, "Use Blink Dagger", "Auto-blink to target when Combo key is pressed.")
SkyWrathHake.IsEZKChecked = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake"}, "Check for EZ Kill", "Check if an enemy is ez-killable (low-mana costs and the fastest way to slay an enemy).")
SkyWrathHake.EZTogglerKey = Menu.AddKeyOption({"Hero Specific", "SkyWrathHake"}, "EZ Kill Toggle Key", Enum.ButtonCode.KEY_V)
SkyWrathHake.IsDoubleUltiToggled = Menu.AddOptionBool({"Hero Specific", "SkyWrathHake"}, "Double ulti mode", "Casts double ulti with aghanim scepter item/buff equiped.")
...
function SkyWrathHake.OnUpdate()
	if not Engine.IsInGame() or Heroes.GetLocal() == nil or not GameRules.GetGameState() == 5 or not Menu.IsEnabled(SkyWrathHake.IsToggled) or GameRules.IsPaused() then return
	end
	...
	if Menu.IsKeyDownOnce(SkyWrathHake.EZTogglerKey) then
		if Menu.IsEnabled(SkyWrathHake.IsEZKChecked) then
			Menu.SetEnabled(SkyWrathHake.IsEZKChecked, false)
		else Menu.SetEnabled(SkyWrathHake.IsEZKChecked, true)
		end		
	end
